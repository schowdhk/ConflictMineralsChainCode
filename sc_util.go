...
func (sc *SmartContract) createAsset(stub shim.ChaincodeStubInterface) pb.Response {
	...
	objIDField := args[0]
	_SC_LOGGER.Info("objIDField: " + objIDField)
...
func (sc *SmartContract) RetriveRecords(stub shim.ChaincodeStubInterface) pb.Response {

	_SC_LOGGER.Info("RetrieveRecords start")
	...
	_SC_LOGGER.Info("Criteria " + criteria)

	records := make([]map[string]interface{}, 0)
	//selectorString = fmt.Sprintf("{\"selector\":{\"status\":\"%s\"}}", criteria)
	...
	for resultsIterator.HasNext() {
		_SC_LOGGER.Info("Records found")
		record := make(map[string]interface{})
		recordBytes, _ := resultsIterator.Next()
		err := json.Unmarshal([]byte(recordBytes.Value), &record)
		if err != nil {
	...
	marshalJSON, err := json.Marshal(records)
	if err != nil {
		_SC_LOGGER.Error("Unable to Marshal Retrieve Records")
		return shim.Error("Unable to Marshal Retrieve Records")
	}

	return shim.Success(marshalJSON)
}

//RetriveRecordsByQuery is the entry point for Query selector
/*func (sc *SmartContract) RetriveRecordsByQuery(stub shim.ChaincodeStubInterface) pb.Response {



	asset := sc.RetriveRecords(stub, criteria)
	if asset == nil {
		return shim.Error("Asset does not exist")
	}
...
func (sc *SmartContract) CheckObjects(input interface{}) bool {
	...
	if ok == true && dataMap["shipmentNumber"] != nil {
		return true
	}
	return false
}
...
func (sc *SmartContract) handleFunctions(stub shim.ChaincodeStubInterface) pb.Response {
	...
	case "getRecordsByShipmentNumber":
		return sc.getAssetDetails(stub)
	case "getRecordsByQuery":
		return sc.RetriveRecords(stub)
	}
